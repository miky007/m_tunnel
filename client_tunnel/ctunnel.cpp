...
std::string GLOBAL_forward_host="";
    ...
    class bridge : public boost::enable_shared_from_this<bridge>
    {
        ...
        private:
            ...
            void handle_upstream_write(const boost::system::error_code& error)
            {
                if (!error)
                {
                    //--> INPUT DATA SSH FROM CLIENT 
                    //PROCESS DATA HERE
                    std::cout <<"### HEADER ###\n"<<   downstream_data_ << "##########\n"<< std::endl;
                    
                    c_http_data hd;

                    std::string rawdata=(char*)downstream_data_;
                    //todo crypt data here
                    //crypt(&rawdata);
                    std::string encoded_data="GET http://" + GLOBAL_forward_host + "/ HTTP/1.1\n\n\n";
                    encoded_data+="Host: "+GLOBAL_forward_host+"\n";
                    encoded_data+="User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:30.0) Gecko/20100101 Firefox/30.0\n";
                    encoded_data+="Accept-Encoding: gzip, deflate\n";
                    encoded_data+="Cookie: "+rawdata+"\n";
                    encoded_data+="\n\n";

                    //send the data to proxy
                    memset(downstream_data_,0,max_data_length);
                    strcpy((char *)downstream_data_, encoded_data.c_str());
                    downstream_socket_.async_read_some(
                             boost::asio::buffer(downstream_data_,encoded_data.size()),
                             boost::bind(&bridge::handle_downstream_read,
                                  shared_from_this(),
                                  boost::asio::placeholders::error,
                                  boost::asio::placeholders::bytes_transferred));
...
int main(int argc, char* argv[])
{
    ...
    GLOBAL_forward_host=forward_host;
